...
#define __LEDS_TEENSY_H__
	...
	/* Public Interface - May be used in end-application: */
		/* Macros: */
			...
			#define LEDS_LED1             (1 << 2) //Red LED on Port B

			/** LED mask for the second LED on the board. */
			#define LEDS_LED2             (1 << 3) //Green LED on Port D

			/** LED mask for all the LEDs on the board. */
			#define LEDS_ALL_LEDS         (LEDS_LED1 | LEDS_LED2) //Set both bits and check further later
		...
		/* Inline Functions: */
			...
			{
				//Both ports required as PS2Chipper's board uses both
				DDRB  |= LEDS_LED1;
				PORTB &= ~LEDS_LED1;
				DDRD  |= LEDS_LED2;
				PORTD &= ~LEDS_LED2;
			...
			{
				PORTB |= (LEDS_LED1 & LEDMask);
				PORTD |= (LEDS_LED2 & LEDMask);
			}
